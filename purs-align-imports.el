;;; purs-align-imports.el --- Align the import lines in a PureScript file
...
;;
;; When purs-align-imports is run within the same buffer, the
...
;; customize `purs-align-imports-pad-after-name', and you'll get:
;;
;; import                  One
;; import                  Two       as A
;; import qualified        Three
...
(defvar purs-align-imports-regexp
  (concat "^\\(import[ ]+\\)"
          "\\(qualified \\)?"
          "[ ]*\\(\"[^\"]*\" \\)?"
          "[ ]*\\([A-Za-z0-9_.']+\\)"
...
(defcustom purs-align-imports-pad-after-name
  nil
  "Pad layout after the module name also."
  :type 'boolean
  :group 'purs-interactive)

;;;###autoload
(defun purs-align-imports ()
  "Align all the imports in the buffer."
  (interactive)
  (when (purs-align-imports-line-match)
    (save-excursion
      (goto-char (point-min))
      (let* ((imports (purs-align-imports-collect))
             (padding (purs-align-imports-padding imports)))
        (mapc (lambda (x)
                (goto-char (cdr x))
                (delete-region (point) (line-end-position))
                (insert (purs-align-imports-chomp
                         (purs-align-imports-fill padding (car x)))))
              imports))))
  nil)

(defun purs-align-imports-line-match ()
...
(defun purs-align-imports-collect ()
  "Collect a list of mark / import statement pairs."
  (let ((imports '()))
    (while (not (or (equal (point) (point-max)) (purs-align-imports-after-imports-p)))
      (let ((line (purs-align-imports-line-match-it)))
        (when line
          (let ((match
                 (purs-align-imports-merge-parts
                  (cl-loop for i from 1 to 8
                           collect (purs-align-imports-chomp (match-string i line))))))
            (setq imports (cons (cons match (line-beginning-position))
                                imports)))))
      (forward-line))
    imports))

(defun purs-align-imports-merge-parts (l)
        ...
        (join (lambda (ls)
                (cl-reduce (lambda (a b)
                             (concat a
                                     ...
                                     b))
                           ls))))
    (if purs-align-imports-pad-after-name
...
(defun purs-align-imports-chomp (str)
...
(defun purs-align-imports-padding (imports)
...
(defun purs-align-imports-fill (padding line)
...
(defun purs-align-imports-line-match-it ()
  "Try to match the current line as a regexp."
  (let ((line (buffer-substring-no-properties (line-beginning-position)
                                              (line-end-position))))
    (if (string-match purs-align-imports-regexp line)
        line
      nil)))

(defun purs-align-imports-after-imports-p ()
...
(provide 'purs-align-imports)

;;; purs-align-imports.el ends here
